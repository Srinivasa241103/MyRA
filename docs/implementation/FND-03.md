...
migrations:
  ...
  - backend/src/database/migrations/0002_fnd_03_integrity_hardening.sql
tests_run:
  ...
  - npm run typecheck
  - npm run build
  - node --import tsx test/test-setup.js
  - npm run test:stream
  - node --import tsx test/test-hybrid-search.ts
manual_validation:
  - Applied 0001 to an empty disposable PostgreSQL database, inserted valid pre-hardening action state, and confirmed 0002 backfilled approval linkage safely.
  - Re-ran both migrations and confirmed the checksum ledger produced no schema mutation.
  - Confirmed User B cannot read or mutate User A runs, steps, evidence, connectors, actions, receipts, or audit history.
  - Confirmed same-user steps, tool calls, evidence, approvals, and idempotency records cannot be linked across different runs or proposals.
  - Confirmed execution cannot be claimed without a matching, exact, non-expired approval.
...
The ordered migrations create the V2 root records for agent runs, steps, tool calls,
evidence, action proposals, approvals, idempotency, action receipts, connector installations, and
audit events, then harden cross-record linkages and exact SHA-256 checks. Database checks use the
FND-01/FND-02 flow, run, tool, evidence, proposal, approval, receipt, and verification values.
...
- rejects an applied migration that disappears from append-only repository history;
...
The action repository always persists a proposal in `proposed`, records an exact-hash approval,
requires that stored non-expired approval before claiming execution under a serializable
transaction, and records the external outcome atomically.
The database uniquely binds `(user_id, idempotency_key)` and one idempotency/receipt record to each
proposal. Every idempotency record carries the matching approval decision. Unknown outcomes
require error and reconciliation metadata; successful outcomes require an external ID, provider
result, and an exact provider-payload hash.
